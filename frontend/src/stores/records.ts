 ...
 *
 * 筛选在**前端**做：契约 §6.4 **已定义** 7 个筛选参数，但**后端尚未实现**
 * （实测 `?rule_hit=3` 仍返回 `rule_hit=1` 的记录，`total` 不随筛选变化）。
 * 在等后端期间，为让「筛选项 + 用响应 total 分页」两个要求同时成立且不越契约，
 * 采用「拉取当前页 → 前端过滤 → 分页条仍显示服务端 total」的口径，
 * 并在页面上如实标注「筛选仅作用于当前页」。
 *
 * ⚠️ **后端就绪后必须切换**（替换点见 `frontend/README.md` §6.1）：
