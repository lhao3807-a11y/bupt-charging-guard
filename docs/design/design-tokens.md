...
> ✅ **已确认采纳（2026-09-13，吕浩）**：车型标签取车牌底色（新能源绿 `#0E7A2E` / 燃油蓝 `#1D4FD8`）。
> 后续前端实现时，`vehicle.vtype` 列一律用 `--plate-*` 令牌，不得改用 `--state-success-*`。
...
> ✅ **已确认保留（2026-09-13，吕浩）**：三档违规类型配三色的映射最自然，第 2 页表格与第 4 页图表都需要一眼分辨，
> 故 `--state-caution-*` **正式保留**为设计系统的一部分，不再是「待评审项」。
> 注意配套约束：`--state-caution-solid` 白底对比度仅 2.38，**只能作有色块填充，不得作白底文字/图形**（`-text` 档承担后者）。
