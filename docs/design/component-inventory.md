...
> **版本** v1.2（2026-09-19 · 新增 §4.0 列名↔契约字段映射表；统一第 2/4 页 `plate` 列名为「车牌」）
> **负责人** 吴和庆（UI / 美术）
...
### 4.0 线框列名 ↔ 契约字段映射（v1.2 新增，前端 `el-table-column` 直接照抄）

线框表头是**中文显示标签**，`prop` 必须用契约字段名。逐条登记如下 ——
**新增列必须同时改这里 + `CONTRACT.md` + `check_tokens.py` 的 `CONTRACT_COLUMNS`**，三处缺一不可。

| 线框 | 列名（显示标签） | 契约表 | 字段 | 契约说明原文 | 备注 |
|---|---|---|---|---|---|
| 第 1 页 | 车牌号 | `vehicle` | `plate` | 车牌号 | |
| 第 1 页 | 车型 | `vehicle` | `vtype` | 车型 | |
| 第 1 页 | 车主 | `vehicle` | `owner` | 车主 | |
| 第 1 页 | 手机号 | `vehicle` | `phone` | 绑定手机号 | 显示标签简写 |
| 第 1 页 | 录入时间 | `vehicle` | `created_at` | 录入时间 | |
| 第 2 页 | ID | `occupation_record` | `id` | 记录 ID | 显示标签简写；**必须展示**（`/api/notify` 入参） |
| 第 2 页 | 车牌 | `occupation_record` | `plate` | 车牌 | |
| 第 2 页 | 车型 | `occupation_record` | `vtype` | 车型 | |
| 第 2 页 | 桩 ID | `occupation_record` | `pile_id` | 桩 ID | |
| 第 2 页 | 命中规则 | `occupation_record` | `rule_hit` | §3.3 该行说明是枚举清单，无短标签 | 标签由设计侧命名，**不是自造字段** |
| 第 2 页 | 命中时间 | `occupation_record` | `occur_time` | 命中时间 | |
| 第 2 页 | 提醒状态 | `occupation_record` | `notify_status` | 提醒状态 | |
| 第 2 页 | 提醒时间 | `occupation_record` | `notify_time` | 提醒时间 | |
| 第 3 页 | 桩 ID | `charging_pile` | `pile_id` | 桩 ID | |
| 第 3 页 | 状态 | `charging_pile` | `status` | 运行状态 | 显示标签简写 |
| 第 3 页 | 绑定车牌 | `charging_pile` | `bound_plate` | 绑定车牌 | |
| 第 3 页 | 开始充电时间 | `charging_pile` | `start_time` | 开始充电时间 | |
| 第 3 页 | 充满时间 | `charging_pile` | `end_time` | 充满时间 | |
| 第 4 页 | 车牌 / 车型 / 桩 ID / 命中规则 / 命中时间 | `occupation_record` | `plate` / `vtype` / `pile_id` / `rule_hit` / `occur_time` | 同第 2 页 | 下钻表，**与第 2 页同写法** |
| 第 5 页 | 参数键 | `system_config` | `key` | 参数键 | |
| 第 5 页 | 参数值 | `system_config` | `value` | 参数值 | |
| 第 5 页 | 说明 | `system_config` | `note` | 说明 | |
| 全部 | 操作 | — | — | — | 非契约字段，纯 UI 操作列 |

**三点口径说明**（评审会问到）：

1. **「显示标签简写」不算新增字段。** 手机号 / 状态 / ID 三处是同一字段的更短显示名，
   契约 §1.1 的约定是「表格列以对应表的字段为准……不另发明」，判据是**字段是否来自契约**；
   允许标签更短，但必须登记在本表里可核对。`prop` 一律用契约字段名。
2. **第 2 页的「车牌」是统一后的结果。** 2026-09-19 终检时发现该页把 `plate` 写成「车牌号」，
   而 `occupation_record` 的契约说明是「车牌」，且第 4 页写的是「车牌」—— 三处分叉。
   已统一为「车牌」，并由 `check_tokens.py` [5] 项的同表跨页断言守住（见 §5 第 9 项）。
3. **第 1 页的「车牌号」保留。** 它对应 `vehicle.plate`，而 `vehicle` 的契约说明原文就是「车牌号」，
   与第 2 页的 `occupation_record.plate`（说明为「车牌」）是**两张表各自的措辞**，不做跨表拉平。
