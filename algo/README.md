...
## 3. 数据集现状（PLAN §2.2 任务 2.2）✅ 基线已就绪

- 目标：4 类场景（燃油占位 / 新能源充电中 / 新能源空闲久停 / 已充满），**每类 ≥30 张，合计 ≥120**。
- 当前基线：**合成数据集 132 张已生成**（`tools/gen_synth_dataset.py`，4 类各 33，train 109 / val 23，
  YOLO 2 类标注 vehicle/plate + `meta.json` 全量场景标注）。
- ⚠️ **合成图只能验证流程跑通，不能代表真实准确率**。校园实拍（每类 ≥30）到位后：
  照片放入 `dataset/images/` 重新标注 → 重训 → 用 `meta.json` 流程回归。
- 训练/推理入口见 `train/detect.py`；车牌识别入口见 `recognize/plate.py`。

### 第 1 周任务完成状态（PLAN §2.2）

| # | 任务 | 状态 |
|---|---|---|
| 2.1 | CV 环境搭建 | ✅ 7 项自检全 PASS |
| 2.2 | 数据集 ≥120 张 | ✅ 合成 132 张（实拍待补） |
| 2.3 | YOLOv8 训练跑通 | ✅ mAP50=0.995（合成集） |
| 2.4 | 车牌识别 + 绿蓝牌判定 | ✅ 链路通；OCR 号码在合成牌上受限，等实拍 |
| 2.5 | 识别桩 stub/real 开关 | ✅ env > system_config，默认 stub |
| 2.6 | 规则引擎边界用例 | ✅ 6 个新用例 |
| 2.7 | 测试与 lint 全绿 | ✅ 后端 71 + algo 21，ruff/black 全绿 |
